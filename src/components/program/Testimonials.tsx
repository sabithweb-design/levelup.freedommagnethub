...
export function ImageTestimonials({ testimonials, title, subtitle }: { testimonials: Testimonial[] } & TestimonialSectionProps) {
  ...
  return (
    <section className={isSubBlock ? "pb-16 md:pb-24 px-6 max-w-7xl mx-auto" : "py-16 md:py-24 px-6 max-w-7xl mx-auto"}>
      {!isSubBlock && (
        ...
        {testimonials.map((t, idx) => (
          <Card key={idx} className="h-full glass-card hover:-translate-y-2 transition-all duration-500 rounded-[3rem] overflow-hidden group flex flex-col border-white/5">
            ...
            <CardContent className="p-8 md:p-10 flex flex-col flex-grow relative">
              <Quote className="absolute top-6 right-6 md:top-8 md:right-8 w-16 h-16 md:w-20 md:h-20 text-white/5 -rotate-12 group-hover:text-accent/10 transition-all duration-500" />
              
              <div className="flex-grow relative z-10">
                <p className="text-white/90 text-xl md:text-2xl font-medium leading-relaxed italic mb-8 md:mb-10">
                  "{t.content}"
                </p>
              </div>
